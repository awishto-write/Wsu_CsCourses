//
// Created by sherryl Dion-Gokan on 12/2/22.
//
...
/********************* Start Microassignment zone ********************/
    ...
    void percolateDown(unsigned int hole)
    {
       ...
       unsigned int child;
        unsigned int current_size = _items.size();
        ...
        {
            child = hole * 2;
            if(child != current_size && _items[child + 1] < _items[child])
            {
                child++;
            }
            if(_items[child] < tmp)
      ...
      //  if(current_size == _items.size() - 1)
       ...
       // unsigned int current_size = size();
        //unsigned int current_size = _items.size();
                       ...
                       // Percolate Up
      unsigned int hole = ++current_size;
