...
class PSOperators:
    ...
    def define(self,name, value):  
        if self.dictstack == []:   # If the dictstack is empty
           ...
           self.dictPush(self.otherDict)  # adding the new dictionary that is updated to dictstack
           
        else:  #if self.dictstack != []:   # When the dictstack is not empty
                self.dictstack[len(self.dictstack)-1][name] = value  # update the top of dictstack
    """
    ...
    def lookup(self,name):  
        name = '/' + name
        for item in reversed(self.dictstack):  # searching at the top of the dictstack
    ...
    def begin(self):  
         ...
         if len(self.opstack) > 0:
            op = self.opPop()  # Pops a DictionaryValue value from opstack
        
            if(isinstance(op,DictionaryValue)):  # checking if that value is a dictionary value
               self.dictPush(op.value)      #pushes it's `value` to the dictstack
            else:
               print("Error: the element value does not have DictionaryValue type")   
