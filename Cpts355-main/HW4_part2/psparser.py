...
def read_expr(src):
    ...
    #       create a PSCodeArray object having this list value.    
    ...
    elif is_literal(token):    # elif(isinstance(token,Literal)):
        return Literal(token)
    
         # if the token is a string delimiter 
    elif token == '(':
       #return PSString(read_str_constant(token))  
       ...
       return PSName(token)
        ...
        # if the token is a code-array delimiter
    elif token == '{':
       #return PSCodeArray(read_block_expr(token)) #src
