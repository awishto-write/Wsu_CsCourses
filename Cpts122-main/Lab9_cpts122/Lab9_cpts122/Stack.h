...
{
	     // mSize can represents int index(i), as we sometimes use
    if(mSize== mMaxSize)      // We do not want the actual size to exceed the max size  // Should I use newSize? instead
	return false;

	 else
	 {
		 mTop[mSize++] = newItem;      // Adding new items on top of the stack  
		 return true;
	 }
}

// In this implementation you will apply defensive design. You must check to see if the stack is empty 
// or not before you pop. Places the popped item in the parameter referred to as "poppedItem".
...
// In this implementation you will apply defensive design. You must check to see if the stack is empty or not 
// before you peek. Places the item at the top of the stack in the parameter referred to as "item".
//  Returns true if there is an item at the top; false otherwise.
template <class T>
