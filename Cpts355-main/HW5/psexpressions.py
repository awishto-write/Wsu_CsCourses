...
class PSString(Expr):
    """A PSString is notation for rep   `resenting a string constant in code. `
...
class PSName(Expr):
    """A `PSName` is a variable , a built-in operator, or a function. 
        a.	If the `PSName` represents a name constant (i.e., its `var_name` attribute starts with a `/`), it will 
        be evaluated to a Python string having value `var_name` . The evaluated value will be pushed onto the opstack.
        b.	If the `PSName` represents a built-in operator (i.e., its `var_name` attribute is one of the built-in operator 
        names),  then we will evaluate it by executing the operator function defined in psoperators.py in the current
        environment (opstack). 
        c.	If the `PSName` represents a variable or function, interpreter looks up the value of the variable in the current 
          environment (dictstack).
    ...
    def getIndex_staticLink(self, ps_env, PName):
        PName = '/' + PName
        for i in range(len(ps_env.dictstack)):
                # Search for the index
            if PName in ps_env.dictstack[i][1]:
                return i
        return None
    ...
    def eval(self,ps_env):
        """`ps_env` is the PSOperators object that include the `opstack` and `dictstack`. """
        """TO-DO (part2)"""  
        PSName = self.var_name
        
           # If the `PSName` represents a name constant
        if(PSName[0] == '/'):
            ps_env.opPush(PSName)
        
            # If the `PSName` represents a built-in operator
        elif(PSName in ps_env.builtin_operators):
           ps_env.builtin_operators[PSName]()
           
            # If the `PSName` represents a variable or function
        else:
             look_value = ps_env.lookup(PSName)
               
               # If the variable value is a code-array (`CodeArrayValue`)
             if(isinstance(look_value, CodeArrayValue)):
                   
                   ps_env.dictPush((self.getIndex_staticLink(ps_env, PSName), {}))
                   look_value.apply(ps_env)
                   ps_env.dictPop()  # When a function execution is done, remember to pop the tuple for that function call from the `dictstack`
                   
             else:   # the variable value is a constant
                   ps_env.opPush(look_value)
